  ...
  useEffect(() => {
    if (isSuccess) {
      const timer = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) { clearInterval(timer); navigate('/estudiante/login?passwordReset=success'); return 0; }
          return prev - 1;
        });
      }, 1000);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 relative" style={backgroundStyle}>
        ...
        <div className="relative max-w-sm w-full bg-white px-8 py-10 sm:px-10 sm:py-12 shadow-2xl border border-zinc-200/50 text-center" style={{ boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
          ...
          <Link to="/estudiante/forgot-password" className="inline-flex items-center justify-center w-full py-3.5 px-4 rounded-none text-white font-medium transition-all duration-200 bg-zinc-700 hover:bg-zinc-600 hover:shadow-lg">
            Solicitar Nuevo Enlace
          </Link>
          <div className="mt-5 text-center">
            <Link to="/estudiante/login" className="inline-flex items-center text-sm font-medium hover:underline text-zinc-600 hover:text-zinc-900 transition-all">
