...
import {
  User, Lock, Calendar, Shield, CheckCircle2,
...
export default function PerfilPage() {
  ...
  const { register: registerPasskey, loading: registeringPasskey } = useWebAuthnRegister();

  const handleRegisterPasskey = async () => {
    if (!user?.email) return;
    console.log('[PerfilPage] Starting passkey registration for:', user.email);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {message && !isModalOpen && (
          <motion.div
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 ${message.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
              }`}
          >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-zinc-200 shadow-sm">
            ...
            <button
              ...
              className="w-full flex items-center justify-between p-3 rounded-xl bg-zinc-50 hover:bg-zinc-100 border border-zinc-200 transition-all group mt-2"
            >
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white border border-zinc-200 flex items-center justify-center text-purple-500 group-hover:text-purple-600 transition-colors">
                  <Fingerprint className="w-4 h-4" />
                </div>
                <div className="text-left">
